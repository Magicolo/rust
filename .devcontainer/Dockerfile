...
RUN apt update
RUN apt upgrade --yes
RUN apt install --yes git git-lfs curl build-essential pkg-config cmake libssl-dev lldb google-perftools
...
RUN cargo install cargo-expand cargo-audit cargo-msrv cargo-hack cargo-semver-checks cargo-llvm-cov
